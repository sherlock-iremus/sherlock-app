...
export default function PredicateWithManyLinkedResources({ n, predicateUri, resourceUri }: {
    ...
    const headerColumnNames: string[] = useMemo(() => ['label', 'linked_resource'].filter(value => data?.head.vars.includes(value)), [data])
    const headerColumns = useMemo(() => headerColumnNames.map(sparqlVar => ({ name: sparqlVar, uid: sparqlVar })), [headerColumnNames])
    ...
    const sortedItems = useMemo(() => {
        return [...filteredItems].sort((a: SparqlQueryResultObject_Binding, b: SparqlQueryResultObject_Binding) => {
            const first = a[sortDescriptor.column as keyof SparqlQueryResultObject_Binding].value as string
            const second = b[sortDescriptor.column as keyof SparqlQueryResultObject_Binding].value as string
