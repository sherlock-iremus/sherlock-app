...
export function displayLabel(v: SparqlQueryResultObject_Variable) {
  ...
  const lang = v['xml:lang'] ? (
    <span className='lang'>{' @' + v['xml:lang']}</span>
  ) : (
    ''
  )
  return (
...
export function makeClickablePrefixedUri(uri: string, pu: PrefixedUri, key: string = '') {
  return (
    <Link to={'/?resource=' + uri}>
      <span className='font-mono whitespace-nowrap' key={key}>
        {
          pu.prefix ?
            <>
              <span>{pu.prefix}</span>
...
export function makeLinkedResourceTypesFragment(
  ...
  const addDivider = () => {
    if (types.length > 0)
      types.push(
        <span className='px-1 text-stone-600' key={getKey()}>
          •
        </span>
      )
  }
  ...
  if (b['r_type_type']) {
    ...
    types.push(
      <Link
        className=''
        key={getKey()}
        to={'/?resource=' + b['r_type_type'].value}
      >
